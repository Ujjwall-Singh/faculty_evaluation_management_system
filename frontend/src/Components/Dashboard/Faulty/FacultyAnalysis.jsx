  ...
  useEffect(() => {
    const fetchFacultyReviews = async (name, department) => {
      try {
        ...
        // Set empty reviews and process empty data to avoid chart errors
        setReviews([]);
        processAnalysisData([]);
      } finally {
        setLoading(false);
    ...
    if (storedFacultyInfo) {
      ...
      // For testing, set some mock faculty info
      const mockFaculty = {
        name: 'Vivek Kumar',
        department: 'Computer Science'
      };
      setFacultyInfo(mockFaculty);
      fetchFacultyReviews(mockFaculty.name, mockFaculty.department);
    }
  }, []);
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-indigo-50 p-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-800">Faculty Performance Analysis</h1>
            <p className="text-gray-600 mt-2">
              Detailed review analysis for {facultyInfo?.name || 'Faculty'} • {facultyInfo?.department || 'Department'}
            </p>
          </div>
        </div>
      </div>
